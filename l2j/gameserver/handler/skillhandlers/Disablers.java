...
public class Disablers implements ISkillHandler
{
	...
	public void useSkill(Creature activeChar, L2Skill skill, WorldObject[] targets)
	{
		...
		for (WorldObject obj : targets)
		{
			...
			// FakePlayer detection (lightweight – avoids imports of custom classes).
			...
			switch (type)
			{
				...
				case CANCEL_DEBUFF:
				{
					...
					int countCd = (skill.getMaxNegatedEffects() > 0) ? 0 : -2;
					for (L2Effect e : effects)
					{
						...
						{
							countCd++;
							if (countCd >= skill.getMaxNegatedEffects())
								break;
						}
					}
					break;
				...
				case NEGATE:
				{
					...
					// Skills with negateId (specific skill ids)
					if (skill.getNegateId().length != 0)
					{
						if (targetIsFakeBot)
						{
							// Manual controlled removal (skip positive non-toggle buffs).
							...
							for (int id : skill.getNegateId())
							{
								...
								for (L2Effect e : effects)
								{
									...
									// Protect positive non-toggle buffs.
									if (!e.getSkill().isDebuff() && !e.getSkill().isToggle())
										continue;
					...
					else
					{
						...
						for (L2Effect e : target.getAllEffects())
						{
							...
							// FakePlayer protection: skip positive non-toggle buffs.
							if (targetIsFakeBot && !effectSkill.isDebuff() && !effectSkill.isToggle())
								continue;
