			...
			else if (_command.startsWith("Quest "))
			{
				if (!activeChar.validateBypass(_command))
					return;
				        ...
				        if (str.length == 1)
				        {
				            // BEGIN: custom quest event support for questId == -1 (custom scripts)
				            net.sf.l2j.gameserver.scripting.Quest q = net.sf.l2j.gameserver.scripting.ScriptManager.getInstance().getQuest(str[0]);
				            if (q != null && q.getQuestId() == -1)
				            {
				                // Call onAdvEvent even if no QuestState (for custom scripts)
				                q.onAdvEvent("", null, activeChar);
				            }
				            else
				            {
				        ...
				        else
				        {
				            ...
				            {
				                // Call onAdvEvent even if no QuestState (for custom scripts)
			               q.onAdvEvent(str[1], null, activeChar);
				           }
				            else
				           {
