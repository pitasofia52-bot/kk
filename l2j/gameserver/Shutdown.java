	...
	public void run()
	{
		if (this == SingletonHolder._instance)
		{
			StringUtil.printSection("Under " + MODE_TEXT[_shutdownMode] + " process");
						...
						// Save PvP bots kills file (for COM/COMPVP bots, safe if not used)
						try {
							com.elfocrash.roboto.pvp.BotKillsStorage.getInstance().save();
							_log.info("BotKillsStorage data has been saved.");
						} catch (Throwable t) {
							_log.log(Level.WARNING, "BotKillsStorage save failed: " + t.getMessage(), t);
						}
