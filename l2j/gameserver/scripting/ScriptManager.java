...
public final class ScriptManager implements Runnable
{
	...
	private final void load()
	{
		try
		{
			...
			for (Node script = n.getFirstChild(); script != null; script = script.getNextSibling())
			{
				...
				try
				{
					// Create the script (Java 17 safe, replaces deprecated Class#newInstance()).
					Class<?> clazz = Class.forName("net.sf.l2j.gameserver.scripting." + path);
					Object obj = clazz.getDeclaredConstructor().newInstance();
					Quest instance = (Quest) obj;
					...
					if (instance instanceof ScheduledQuest)
					{
						...
						if (param == null)
						{
							_log.warning("ScriptManager: Missing \"start\" parameter for \"" + path + "\".");
							continue;
						}
				...
				{
					_log.warning("ScriptManager: Failed to instantiate script \"" + path + "\": " + e);
					continue;
				}
			}
	...
	private final class Scheduler implements Runnable
	{
		...
		public void run()
		{
			...
			// in case the next action is triggered before the resolution of ScriptManager, schedule the action again
