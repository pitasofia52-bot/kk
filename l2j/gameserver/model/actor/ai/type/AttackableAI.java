...
public class AttackableAI extends CreatureAI implements Runnable
{
	...
	protected void thinkAttack()
	{
		...
		Creature attackTarget = npc.getMostHated();
		        ...
		        // Leash μόνο για Monster: αν απομακρυνθεί >300 από το spawn, σπάει το aggro και επιστρέφει σπίτι.
		        if (npc instanceof net.sf.l2j.gameserver.model.actor.instance.Monster)
		        {
		            if (npc.getSpawn() != null
		                    && !npc.isInsideRadius(
		                            npc.getSpawn().getLocX(),
		                            npc.getSpawn().getLocY(),
		                            npc.getSpawn().getLocZ(),
		                            1000, true, false))
		            {
		                npc.getAggroList().clear();
		                npc.setIsReturningToSpawnPoint(true);
		                npc.setWalking();
		                setIntention(CtrlIntention.MOVE_TO, npc.getSpawn().getLoc());
		                return;
		            }
		        }
