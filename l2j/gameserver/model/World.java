...
public final class World
{
	...
	{
				// Αρχικό γρήγορο lookup μέσω PlayerInfoTable.
				Player player = _players.get(PlayerInfoTable.getInstance().getPlayerObjectId(name));
				if (player != null)
					return player;
				
				// Fallback: case-insensitive σάρωση σε τυχόν bots που μετονομάστηκαν μετά το αρχικό registration.
				final String lname = name.toLowerCase();
				for (Player p : _players.values())
				{
					if (p.getName().equalsIgnoreCase(lname))
					{
						// Προσπάθησε να επανακαταχωρήσεις σωστά το mapping στο PlayerInfoTable ώστε τα επόμενα lookups να είναι O(1).
						try
						{
							String acc = (p.getClient() != null) ? p.getClient().getAccountName() : "AutoPilot";
							PlayerInfoTable.getInstance().addPlayer(p.getObjectId(), acc, p.getName(), p.getAccessLevel().getLevel());
						}
						catch (Exception e)
						{
							// Σκόπιμα σιωπηλό για να μην spamάρει – αν χρειαστεί βάλε log fine.
						}
						return p;
					}
				}
