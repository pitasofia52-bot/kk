...
public final class ThreadPool
{
	...
	public static ScheduledFuture<?> schedule(Runnable r, long delay)
	{
		try
		{
						if (_scheduledPools == null)
							{
								LOG.warning("ThreadPool.schedule() called before ThreadPool.init() – task LOST (r=" + r + ", delay=" + delay + ")");
								return null;
							}
		...
		{
			LOG.warning("ThreadPool.schedule() exception: " + e.getMessage());
			return null;
		}
	}
	...
	public static ScheduledFuture<?> scheduleAtFixedRate(Runnable r, long delay, long period)
	{
		try
		{
						if (_scheduledPools == null)
							{
								LOG.warning("ThreadPool.scheduleAtFixedRate() called before ThreadPool.init() – task LOST (r=" + r + ", delay=" + delay + ", period=" + period + ")");
								return null;
							}
		...
		{
			LOG.warning("ThreadPool.scheduleAtFixedRate() exception: " + e.getMessage());
			return null;
		}
	}
	...
	{
		        if (_instantPools == null)
			        {
			            LOG.warning("ThreadPool.execute() called before ThreadPool.init() – task DISCARDED (r=" + r + ")");
			            return;
			        }
			
			        final ThreadPoolExecutor pool = getPool(_instantPools);
			       if (pool == null || pool.isShutdown() || pool.isTerminated())
			        {
			            // Avoid submitting to a terminated executor during shutdown to prevent log spam.
			            return;
			       }
			        ...
			        {
			            // If the pool isn't shutting down, log the issue; otherwise ignore (expected during shutdown).
			            if (!pool.isShutdown() && !pool.isTerminated())
			            {
			               LOG.warning("ThreadPool.execute() exception: " + e.getMessage());
			            }
			        }
			        catch (Exception e)
			        {
